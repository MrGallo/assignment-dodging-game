...
def draw():
    ...
    n = noise(frameCount * 0.005)
    print(n)
    inc = map(n, 0, 1, -0.5, 0.5)
    ball_1_speed_x += inc
        
    # Update ball 1's location
    ball_1_pos_y += ball_1_speed_y
    ball_1_pos_x += ball_1_speed_x
    
    if 0 > ball_1_pos_x > width:
        ball_1_speed_x *= -1
        print(ball_1_speed_x)
    ball_1_pos_x = constrain(ball_1_pos_x, 0, width)
